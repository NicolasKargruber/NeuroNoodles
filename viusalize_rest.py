...
# Load the NIfTI file
#img = nib.load('anat_rpi_blocked_blocked.nii')
...
# Axial plane (z-axis)
axes[0].imshow(data[:, :, num_slices // 2, 1], cmap='gray')
axes[0].set_title('Axial')
axes[0].axis('off')

# Sagittal plane (x-axis)
axes[1].imshow(data[:, num_slices // 2, :, 1], cmap='gray')
axes[1].set_title('Sagittal')
axes[1].axis('off')

# Coronal plane (y-axis)
axes[2].imshow(data[num_slices // 2, :, :, 1].T, cmap='gray', origin='lower')
